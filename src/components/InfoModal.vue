...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-sm transition-opacity" @click.self="emit('close')">
    <div class="bg-slate-800 border border-slate-700 p-6 md:p-8 rounded-xl max-w-md w-full shadow-2xl relative">
      <button @click="emit('close')" class="absolute top-4 right-4 text-slate-500 hover:text-white transition-colors">✕</button>
      ...
      <div class="space-y-4 text-slate-200 text-sm leading-relaxed">
        <p>
          ...
          El cronómetro cuenta hacia arriba <em class="text-slate-300">(solo minutos, para evitar distracciones)</em>. Trabaja hasta cansarte.
        </p>
        <p>
          <strong class="text-teal-200">2. Descansa (Ratio 5:1):</strong>
          5 min trabajo = 1 min descanso. El tiempo se acumula y se muestra <em class="text-slate-300">después de registrar tu sesión</em>.
        </p>
        <p>
        ...
        <div class="border-t border-slate-700 pt-4 mt-4">
          <p class="text-slate-300 text-xs leading-relaxed">
            <strong class="text-slate-200">Límite Cognitivo:</strong>
        ...
        <p class="text-yellow-300 text-xs border-l-2 border-yellow-500/30 pl-3">
